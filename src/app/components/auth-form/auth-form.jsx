...
export const AuthFormComponent = ({
  ...
  const onRegister = () => {
    userCredentials.displayName ? registerUser(userCredentials) : setIsRegistration(true)
  }
    ...
    <Card elevation={2} className={classes.formWrapper}>
      <form className={classes.form} onSubmit={onLogin}>
        ...
        <Box className={classes.socialWrapper}>
          <SocialButton onClick={loginWithGoogle} name='Facebook' icon={GoogleIcon} />
          <SocialButton onClick={loginWithFacebook} name='Facebook' icon={FacebookIcon} />
          <SocialButton onClick={loginWithGithub} name='Facebook' icon={GithubIcon} />
        </Box>
      </form>
    </Card>
  )
